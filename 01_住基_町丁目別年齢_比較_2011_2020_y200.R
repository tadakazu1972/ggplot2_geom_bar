...
#住民基本台帳csv読込　市HPから取得
data1 <- read.csv("./住基データ/02-20_jyu-ki(suminoe)----H23_3.csv") #2011
data2 <- read.csv("./住基データ/02-20_jyu-ki(suminoe)----R02_3.csv") #2020
...
for(j in 1:last){
  quartz(type="png", file=sprintf("住之江区住基年齢構成201103_202003_%d%s.png",j, name[j]), dpi=144, bg="white")
  ...
  #タイトル
  g1 <- g1 + ggtitle("住之江区　年齢別人口(住民基本台帳)") + theme(plot.title = element_text(hjust = 0.5))
  #町丁目　描画
