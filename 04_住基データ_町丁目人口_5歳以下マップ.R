...
#5歳以下を足し算して列を追加
child <- apply(data2[,8:13], 1, sum)
data2 <- data2 %>% mutate("5歳以下" = child )
...
#pngファイル版
{
  ...
  #色を固定する場合の記述
  col_km <- data[[column[144]]] %>% classIntervals(., 10, style="fixed", fixedBreaks=c(10,25,50,75,100,200,300,400,500,1000,max(.))) %>% findColours(.,pal=brewer.pal(10,"Greens"))

  #境界
  plot(st_geometry(shape[7]), col=col_km, main=paste(kuname, "　人口　5歳以下 (住民基本台帳 2020年3月末現在)", sep=""))
...
#色はK-meansで自動計算する場合の記述
col_km <- data[[column[144]]] %>% classIntervals(., 10, style="kmeans") %>% findColours(.,pal=brewer.pal(10,"Greens"))

#色を固定する場合の記述
#col_km <- data[[column[144]]] %>% classIntervals(., 10, style="fixed", fixedBreaks=c(100,200,300,400,500,600,700,800,900,1000,max(.))) %>% findColours(.,pal=brewer.pal(10,"Greens"))
